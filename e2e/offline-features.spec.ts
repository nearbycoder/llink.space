...
test('visitor reading list persists, removes links, and excludes unpublished items',async({page})=>{
 const {username}=await setupCreator(page);const link=await api(page.request,'links.add',{title:'Save this essay',url:'https://example.com/essay'});await page.goto('/u/'+username);const save=page.getByRole('button',{name:'Save Save this essay for later',exact:true});await expect(save).toBeEnabled();await save.click();await expect(save).toHaveAttribute('aria-pressed','true');await page.reload();await expect(save).toHaveAttribute('aria-pressed','true');await page.getByText('Reading list · 1',{exact:true}).click();await page.getByRole('button',{name:'Remove Save this essay from reading list'}).click();await expect(save).toHaveAttribute('aria-pressed','false');await save.click();await api(page.request,'links.bulkAction',{ids:[link.id],action:'pause'});await page.reload();await expect(page.getByText('Reading list · 0',{exact:true})).toBeVisible();await expect(page.getByRole('button',{name:'Save Save this essay for later',exact:true})).toHaveCount(0);
});

test('visitor reading list falls back visibly when browser storage is unavailable',async({page})=>{
 const {username}=await setupCreator(page);await api(page.request,'links.add',{title:'Session essay',url:'https://example.com/session'});await page.addInitScript(()=>Object.defineProperty(window,'localStorage',{get(){throw new Error('Storage blocked');}}));await page.goto('/u/'+username);await expect(page.getByRole('status').filter({hasText:'Browser storage is unavailable'})).toBeVisible();await page.getByRole('button',{name:'Save Session essay for later',exact:true}).click();await expect(page.getByText('Reading list · 1',{exact:true})).toBeVisible();await page.getByText('Reading list · 1',{exact:true}).click();await page.getByRole('button',{name:'Clear reading list',exact:true}).click();await expect(page.getByText('Reading list · 0',{exact:true})).toBeVisible();
});
